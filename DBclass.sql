...
-- 테이블에 데이터 저장
insert into table1 (col1, col2) value(1, '안녕');
insert into table1 (col1, col2) value(1, '화요일');
-- 정의한 컬럼에 맞지 않는 데이터가 들어오면 에러
insert into table1 (col1, col2) value('aa', '화요일');
-- 
insert into table1 (col1, col2) value(4, '화요일입니다');
insert into table1 (col1, col2) value(5, 'abcde');
...
/*
    ...
    학번(studentName),
    학번(studentMobile),
    학번(studentMajor) 정보를 저장하는 student_table 생성
*/
...
-- truncate() 버림 함수 ㅍㅅㅍ 이것도 외워둬라 태송아  -- 넴
select truncate(123.12, 1) from dual; 
...
-- 그룹화한 결과에 조건을 적용
-- having
-- 급여가 3000 이하인 사원만 가지고  평균 급여가 2000 이상인 결과만 조회
...
select deptno, round(avg(sal), 1) as '평균급여', max(sal) as '최고급여', min(sal) as '최저급여', count(*) as '사원수' from emp group by deptno;
select job, count(*) from emp group by job having count(*) >= 3;
select date_format(hiredate, '%Y') as '입사년도', count(*) from emp group by date_format(hiredate, '%Y');
select date_format(hiredate, '%Y') as '입사년도', deptno, count(*) from emp group by date_format(hiredate, '%Y'), deptno;
...
-- deptno 컬럼은 두 테이블에 모두 있기 때문에 ambiguous 에러가 날 수 있음
-- emp 테이블 전체 조회
select empno, e.ename, e.job, e.deptno, d.dame, d.loc from emp e, dept d where e.deptno = d.deptno;
...
select e.*, d.dname, d.loc from emp e, dept d where sal > (select avg(sal) from emp) and e.deptno=20;
...
-- id를 제외하고 나머지 값만 입력
insert into member2 (member_email, member_password) values('cc@cc.com', '1234');
...
create table child1(
    ...
    insert into child1 (id, c1, c2, p_id) values (2, 'aaa', 'aaa', 2);
    insert into child1 (id, c1, c2, p_id) values (3, 'aaa', 'aaa', 3);
    
    -- 부모테이블의 데이터 삭제
    -- id=2인 데이터 한 줄을 삭제
    -- 자식테이블에 id=2인 데이터를 참조하는 부분이 있기 때문에 삭제 불가
    delete from parent1 where id=2;
    -- 자식테이블에 id=4인 데이터를 참조하는 부분이 있기 때문에 삭제 가능
...
select * from customer, orders where customer.orders = customer.orders;
select * from customer c, orders o where c.orders = o.orders;
-- 23. 고객, 주문 테이블 조인하여 고객번호 순으로 정렬
select * from customer, orders where customer.id = orders.customer_id order by customer_id;
-- 24. 고객이름(CUSTOMER), 고객이 주문한 도서 가격(ORDERS) 조회 
select c_name, o_saleprice from orders o, customer c where c.id= o.customer_id;
-- 25. 고객별(GROUP)로 주문한 도서의 총 판매액(SUM)과 고객이름을 조회하고 조회 결과를 가나다 순으로 정렬 
select sum(o_saleprice), c_name from orders o, customer c where c.id=o.customer_id group by c_name order by c_name asc;
...
-- 회원 기능
-- 1. 회원가입(임의의 회원3명 가입)
insert into member_table (member_email, member_name, member_password) values('aa@aa.com', '김xx', 'aaa');
insert into member_table (member_email, member_name, member_password) values('bb@bb.com', '박xx', 'bbb');
insert into member_table (member_email, member_name, member_password) values('cc@cc.com', '이xx', 'ccc');
-- 2. 이메일 중복체크 
select member_email from member_table where member_email='aa@aa.com';
-- 3. 로그인
select * from member_table where member_email='bb@bb.com' and member_password='bbb';
-- 4. 전체 회원 목록 조회 
select * from member_table;
-- 5. 특정 회원만 조회 
select member_name from member_table where member_email='aa@aa.com';
select member_name from member_table where id=1;
...
select m.m_no, m.m_name, p.p_code,
        ...
        concat (p.p_tell, '-', p.p_tel2, '-', p.p_tel3) as '대표전화'
