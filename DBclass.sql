...
create table child2(
    ...
    -- 외래키 지정 (p_id 컬럼을 parent1 테이블의 id 컬럼을 참조하도록 함)
    -- 부모데이터 삭제시 자식데이터도 삭제
    constraint fk_child2 foreign key(p_id) references paren2 (id) on delete cascade
    );
    ...
    insert into child2 (id, c1, c2, p_id) values (2, 'aaa', 'aaa', 2);
    insert into child2 (id, c1, c2, p_id) values (3, 'aaa', 'aaa', 3);
    insert into child2 (id, c1, c2, p_id) values (4, 'aaa', 'aaa', 5);
    select * from child2;
...
create table child3(
    ...
    p_id bigint, -- 참조할 컬럼
    -- 외래키 지정 (p_id 컬럼을 parent1 테이블의 id 컬럼을 참조하도록 함)
    -- 부모데이터 삭제시 자식데이터는 유지되지만 
    constraint fk_child3 foreign key(p_id) references parent3 (id) on delete cascade
    );
    ...
    insert into child3 (id, c1, c2, p_id) values (2, 'aaa', 'aaa', 2);
    insert into child3 (id, c1, c2, p_id) values (3, 'aaa', 'aaa', 3);
    insert into child3 (id, c1, c2, p_id) values (4, 'aaa', 'aaa', 5);
    select * from child3;
    
    -- 수정쿼리
update child3 set c1='수정내용' where id=2; -- pk를 기준으로
update child3 set c1='수정내용', c2='ㅎㅎㅎ' where id=3; -- pk를 기준으로
...
select * from book where b_bookname
